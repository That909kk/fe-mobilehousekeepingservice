...
// Response type for Customer/Employee features
...
export const permissionService = {
  ...
  // GET /api/v1/customer/{customerId}/features - Lấy features của customer
  getCustomerFeatures: async (customerId: string): Promise<UserFeaturesResponse> => {
    const response = await apiClient.get<UserFeaturesResponse>(`/customer/${customerId}/features`);
    return response.data;
  },

  // GET /api/v1/employee/{employeeId}/features - Lấy features của employee
  getEmployeeFeatures: async (employeeId: string): Promise<UserFeaturesResponse> => {
    const response = await apiClient.get<UserFeaturesResponse>(`/employee/${employeeId}/features`);
    return response.data;
  },
